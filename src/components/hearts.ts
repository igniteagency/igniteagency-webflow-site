...
function initHeartsAnimation() {
  ...
  const backgroundCanvas = document.querySelector<HTMLCanvasElement>('#reviews-hearts-bg-canvas');
  const foregroundCanvas = document.querySelector<HTMLCanvasElement>('#reviews-hearts-fg-canvas');

  // Component trigger: Only run if at least one of the required canvas IDs exists on the page
  if (!backgroundCanvas && !foregroundCanvas) {
    return;
  }

  // Set dimensions for existing canvases
  if (backgroundCanvas) {
  ...
  // Get contexts for existing canvases
  ...
  class FloatingHeart {
    ...
    draw() {
      ...
      if (!ctx) return; // In case this layer's canvas doesn't exist
  ...
  function animate(timestamp: number) {
    ...
    if (elapsed > 16) {
      ...
      // Clear both canvases if they exist
      if (bgCtx && backgroundCanvas) bgCtx.clearRect(0, 0, backgroundCanvas.width, backgroundCanvas.height);
      if (fgCtx && foregroundCanvas) fgCtx.clearRect(0, 0, foregroundCanvas.width, foregroundCanvas.height);
